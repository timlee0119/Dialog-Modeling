...
class bert_model():
    def __init__(self, pre_name, epoch = 100, batch_size = 32, lr = 1e-4, valepo = 1, valset = None):
        ...
        self.val_accu_list = []
        self.lr = lr
        self.val_fine_tuned_epo = valepo
        self.model = None
    ...
    def fit_and_train(self, train_df, val_df, val_train_df, require_grad):
        ...
        tokenizer = BertTokenizer.from_pretrained(self.pre_name, do_lower_case=True)
        ...
        model = BertForSequenceClassification.from_pretrained(self.pre_name, num_labels=NUM_LABELS)
   ...
   # (tensor(0.6968, grad_fn=<NllLossBackward>), tensor([[-0.0359, -0.0432]], grad_fn=<AddmmBackward>))
            ...
            with torch.no_grad():
                for data in valloader:
                    if self.gpu:
                        tokens_tensors, segments_tensors, masks_tensors, _ = [x.type(torch.LongTensor).cuda() if x is not None else None for x in data]
                    else:
        ...
        torch.save(best_model.state_dict(), f'../model/bert_tuned_torch_dict') 
